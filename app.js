...
function addMapZoomControls(){ const controls=document.createElement('div'); controls.className='map-zoom-controls'; controls.innerHTML='<button type="button" data-map-zoom="in" aria-label="지도 확대">＋</button><input id="map-zoom-range" aria-label="지도 배율" type="range" min="6" max="18" step="1" value="8" orient="vertical" /><button type="button" data-map-zoom="out" aria-label="지도 축소">−</button>'; controls.addEventListener('pointerdown',event=>event.stopPropagation()); controls.addEventListener('click',event=>{const button=event.target.closest('[data-map-zoom]');if(!button)return;event.preventDefault();event.stopPropagation();map.setZoom(Math.max(6,Math.min(18,map.getZoom()+(button.dataset.mapZoom==='in'?1:-1))));}); controls.querySelector('#map-zoom-range').addEventListener('input',event=>map.setZoom(Number(event.currentTarget.value))); $('#map').append(controls); }
...
function visibleSpots(){ const q=$('#search-input').value; return spots.filter(s=>(filter==='all'||(filter==='community'?isUserSpot(s):s.kind===filter))&&(!mapRegionFilter||provinceOf(s)===mapRegionFilter)&&SpotRegions.matches(s,q)); }
function regionOf(spot){ return SpotRegions.location(spot).district; }
function provinceOf(spot){ return SpotRegions.location(spot).province; }
...
async function init(){
  try { await loadNaverMaps(); }
  catch(error) { $('#map').innerHTML='<p class="map-load-error">네이버 지도를 불러오지 못했습니다. API 서비스와 Web 서비스 URL 설정을 확인해주세요.</p>'; return; }
  map=new naver.maps.Map('map',{center:new naver.maps.LatLng(37.47,127.0),zoom:8,zoomControl:false});
  addMapZoomControls();
  const refreshMapLayout=()=>map?.refresh();
  new ResizeObserver(refreshMapLayout).observe($('#map'));
  window.addEventListener('resize',()=>setTimeout(refreshMapLayout,0));
  infoWindow=new naver.maps.InfoWindow({borderWidth:0,backgroundColor:'transparent',disableAnchor:true,pixelOffset:new naver.maps.Point(0,-30)});
  renderSpots(); renderRanking();
  naver.maps.Event.addListener(map,'zoom_changed',()=>{ const range=$('#map-zoom-range'); if(range) range.value=String(map.getZoom()); refreshMapMarkers(); });
  $('#search-input').addEventListener('input',renderSpots);
  document.querySelectorAll('.chip').forEach(button=>button.addEventListener('click',()=>{ filter=button.dataset.filter; document.querySelector('.chip.selected').classList.remove('selected'); button.classList.add('selected'); renderSpots(); }));
  $('#locate-btn').addEventListener('click',()=>navigator.geolocation ? navigator.geolocation.getCurrentPosition(position=>{ setMapView(position.coords.latitude,position.coords.longitude,14); toast('현재 위치로 이동했어요.'); },()=>toast('위치를 가져올 수 없어 서울 중심 지도를 표시합니다.')) : toast('이 브라우저에서는 위치 정보를 지원하지 않습니다.'));
...
async function loadSpotPosts(spotId) {
  ...
  try {
    ...
    const recommendationsAvailable = result.postRecommendationsAvailable !== false;
    container.innerHTML = posts.length ? posts.map(post => { const imageUrl=safeImageUrl(post.image_url); const ownPost=viewerId && post.author_id === viewerId; const recommended=recommendedPostIds.has(String(post.id)); const recommendationCount=Number(recommendationCounts[String(post.id)] || 0); const recommendationAction=recommendationsAvailable?`<button data-recommend-post="${post.id}" aria-pressed="${recommended}">${recommended ? '♥ 추천했어요' : '♡ 추천'}</button>`:'<small>추천 준비 중</small>'; return `<article class="post-card${imageUrl?' has-image':''}"><div class="post-copy"><header><b>${escapeHTML(post.author || '익명 낚시꾼')}</b><span>${post.length ? `${Number(post.length).toFixed(1)}cm${post.length_is_ai ? ' · AI 추정' : ''}` : '일반 후기'}</span></header><p>${escapeHTML(post.content)}</p><footer><span>${post.species ? escapeHTML(post.species) : '조황 정보'} · 추천 ${recommendationCount}</span><span>${recommendationAction} <button class="report-post" data-report-post="${post.id}">신고</button>${ownPost ? ` <button class="edit-post" data-edit-post="${post.id}">수정</button> <button class="delete-post" data-delete-post="${post.id}">삭제</button>` : ''}</span></footer></div>${imageUrl?`<a class="post-thumbnail" href="${escapeHTML(imageUrl)}" target="_blank" rel="noopener"><img src="${escapeHTML(imageUrl)}" alt="게시글 사진" /></a>`:''}</article>`; }).join('') : '<p class="empty-post">아직 게시글이 없습니다. 첫 조황을 남겨보세요.</p>';
