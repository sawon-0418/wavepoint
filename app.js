...
function addMapZoomControls(){ const controls=document.createElement('div'); controls.className='map-zoom-controls'; controls.innerHTML='<button type="button" data-map-zoom="in" aria-label="지도 확대">＋</button><button type="button" data-map-zoom="out" aria-label="지도 축소">−</button>'; controls.addEventListener('click',event=>{const button=event.target.closest('[data-map-zoom]');if(!button)return;event.preventDefault();event.stopPropagation();map.setZoom(map.getZoom()+(button.dataset.mapZoom==='in'?1:-1));}); $('#map').append(controls); }
...
async function init(){
  ...
  naver.maps.Event.addListener(map,'zoom_changed',refreshMapMarkers);
