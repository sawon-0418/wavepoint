...
- Render Web Service에는 `RUN_IN_PROCESS_SYNC=false`를 설정한다. 공식 데이터 갱신은 별도 Cron Job이 담당한다.
...
- Render Cron Job을 `30 18 * * *`(UTC, 매일 03:30 KST)로 설정하고, Build Command는 `python3 -m py_compile server.py sync_job.py`, Start Command는 `python3 sync_job.py`로 지정한다.
- Cron Job에도 Web Service와 동일한 Supabase·공공데이터·네이버 지도 환경 변수를 등록하고, 첫 실행 로그에서 `공식 데이터 동기화가 완료됐습니다.`를 확인한다.
